function channelDelete(Discord, client, guild, fs, decache) {

    var guildName = guild.name;
    var guildID = guild.id;
    var unlinkedGuildDir = unlinkedGuildDir + guildID + "/";
  
    console.log(` ${client.user.tag} vient d'être retiré du serveur (salon) ${guildName} (${guildID})`)
    ...
    networksList.forEach(network => {
        for(var i = 0; i < network; i++){
            var networkDir = networksDir + network[i] + "/";
